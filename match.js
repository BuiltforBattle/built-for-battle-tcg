async function startMatch() {
      ...
      alert("Start Match button was clicked! 🎯");
      ...
      if (matchMode === "join") {
        ...
        const { data, error } = await supabase
          ...
          .eq("id", matchId)
          .single();
        alert("Attempted to create match with Supabase");
  ...
  // ✅ Move to gameplay screen after either create or join
    ...
    async function testSupabaseInsert() {
      alert("🧪 testSupabaseInsert() called");
      const { data, error } = await supabase
        .from("matches")
        .insert([
          {
            team_data: { 1: [], 2: [] },
          stats: { 1: {}, 2: {} },
            round_winners: ["Pending", "Pending", "Pending"],
            current_turn: 1,
            current_round: 1,
            match_over: false
          }
      ...
      if (error) {
        ...
        alert("✅ Insert Succeeded! Match ID: " + data.id);
